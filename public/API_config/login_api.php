<?php
    ...
    if( $result_1[0] == 0){
        exit(json_encode(['state'=>0, 'msg'=> 'miss'. $result_1[0] ]));
    }else{
        ...
        if( $result_2[0]==0){
            ...
            $data_show = 'SELECT students.*,documents.typestudent FROM `students` LEFT JOIN `documents` ON students.id = documents.student WHERE `stdId` = "'.$data['username'].'" AND `peopleId` = "'.$data['password'].'"';
